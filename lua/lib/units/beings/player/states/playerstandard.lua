...
function PlayerStandard:_get_intimidation_action(prime_target, char_table, amount, primary_only, detect_only, secondary)
	...
	if prime_target then
		if prime_target.unit_type == unit_type_teammate then
			...
			if not detect_only then
				...
				if record.ai then
					if not prime_target.unit:brain():player_ignore() then
						if secondary then --secondary = stop ai command (default keybind is Z)
							if prime_target.unit:movement()._should_stay then --if the target bot was already ordered to stop
								do_nothing = true
							end
						else
							if self._ext_movement:rally_skill_data() and not managers.player:has_enabled_cooldown_upgrade("cooldown", "long_dis_revive") and (prime_target.unit:character_damage():arrested() or prime_target.unit:character_damage():need_revive()) then --if Inspire ace is on cooldown and the target bot is arrested/downed, do nothing
								do_nothing = true
							end
						end

						if not do_nothing then --interact as usual
				...
				if rally_skill_data and mvector3.distance_sq(self._pos, record.m_pos) < rally_skill_data.range_sq then
					...
					if not secondary then
						...
						if needs_revive then
							...
							else --if Inspire ace is on cooldown, do nothing
								do_nothing = true
							end
						elseif is_human_player and not is_arrested and not needs_revive and rally_skill_data.morale_boost_delay_t and rally_skill_data.morale_boost_delay_t < managers.player:player_timer():time() then --added a player check at the beggining so that Inspire basic isn't wasted on bots
							voice_type = "boost"
							amount = 1
						end
					end
			...
			voice_type = voice_type or secondary and not is_human_player and not do_nothing and "ai_stay" or not do_nothing and "come" or nil --added a human player check for "ai_stay" so that it only works with the team AI and made it so it's possible to do (and say) nothing, instead of defaulting to "come"
			plural = false
		else
			...
			if prime_target.unit_type == unit_type_enemy then
				...
				elseif prime_target.unit:base():char_tweak().priority_shout then
					...
					if prime_target.unit:base():char_tweak().surrender and not prime_target.unit:base():char_tweak().surrender.special then --if the enemy can actually surrender, intimidate as usual (stops you from yelling at gangsters, boss enemies, etc, which will never surrender)
						voice_type = "stop_cop"
					end
				end
			elseif prime_target.unit_type == unit_type_camera then --rest of the function below this line remains untouched
			...
			if self._unit:movement():team().foes[u_data.unit:movement():team().id] and not u_data.unit:anim_data().hands_tied and not u_data.unit:anim_data().long_dis_interact_disabled and (not u_data.unit:character_damage() or not u_data.unit:character_damage():dead()) and (u_data.char_tweak.priority_shout or not only_special_enemies) then
				if managers.groupai:state():whisper_mode() then
					...
					elseif not u_data.unit:movement():cool() then
						if u_data.char_tweak.surrender and not u_data.char_tweak.surrender.special then --enemies that can't surrender won't be even considered as targets to yell at, which would prevent you from considering other targets if they're in the way
		...
		for u_key, u_data in pairs(criminals) do
			...
			if u_key ~= self._unit:key() then
				...
				if rally_skill_data and rally_skill_data.long_dis_revive and mvector3.distance_sq(self._pos, u_data.m_pos) < rally_skill_data.range_sq then
					...
					if needs_revive then
						if managers.player:has_enabled_cooldown_upgrade("cooldown", "long_dis_revive") then --downed teammates won't even be considered as targets to yell at if Inspire ace is on cooldown, which would prevent you from considering other targets if they're in the way
							added = true
			...
			if not added and not u_data.is_deployable and not u_data.unit:movement():downed() and not u_data.unit:base().is_local_player and not u_data.unit:anim_data().long_dis_interact_disabled then
				if secondary then
					if not u_data.unit:base().is_husk_player and not u_data.unit:movement()._should_stay then --players, and bots (that already have the order to stay put) won't even be considered as targets for secondary shouts, allowing you to easily tell all 3 bots to stay, even if they're one behind the other
