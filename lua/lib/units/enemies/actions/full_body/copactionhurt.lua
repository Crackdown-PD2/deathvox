...
function CopActionHurt:init(action_desc, common_data)
	...
	elseif action_type == "light_hurt" then
		if (self._unit:anim_data() and self._unit:anim_data().act) then --prevent flinching while acting
			return
		end
	...
	if self._ext_inventory and not self._weapon_dropped and common_data.char_tweak.shooting_death and not self._ext_movement:cool() then --uncool enemies always fire their weapons (the vanilla 3-second time-window is never conveyed to the player)
		local weapon_unit = self._ext_inventory:equipped_unit()
	...
	if not self._unit:base().nick_name then
		if action_desc.variant == "fire" then --just proper checks here
			if action_desc.hurt_type == "fire_hurt" then
				local immune_to_firehurt = self._unit:base():has_tag("tank") or self._unit:base():has_tag("spooc") or self._unit:base():has_tag("medic") or self._unit:base():has_tag("shield")

				if not immune_to_firehurt then
		...
		if self._unit:base():has_tag("tank") and action_type == "death" then --include ALL Dozers for the OVERGRILL achievement
...
function CopActionHurt:chk_block(action_type, t) --prevent hurt animations from being triggered until the expire time for the ones below runs out (prevents flinching while in a hurt animation + stunlock loops like Shock and Awe against Shields)
