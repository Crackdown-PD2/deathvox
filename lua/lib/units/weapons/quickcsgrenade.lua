...
function QuickCsGrenade:_setup_from_tweak_data()
	...
	self._radius = 400
	self._radius_blurzone_multiplier = 1.3
	self._damage_tick_period = 1
	self._damage_per_tick = 3
end
...
function QuickCsGrenade:_do_damage()
	...
	if player_unit and mvector3.distance_sq(self._unit:position(), player_unit:position()) < self._radius * self._radius then
		local attack_data = {
