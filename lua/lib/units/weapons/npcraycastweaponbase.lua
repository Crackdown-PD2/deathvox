Hooks:PostHook(NPCRaycastWeaponBase,"init","cd_newraycastweaponbase_init",function(self,...)
	self._bullet_slotmask = self._bullet_slotmask - World:make_slot_mask(22)
end)

Hooks:PostHook(NPCRaycastWeaponBase,"setup","cd_newraycastweaponbase_setup",function(self,setup_data,...)
	self._bullet_slotmask = self._bullet_slotmask - World:make_slot_mask(22)
...
function NPCRaycastWeaponBase:_fire_raycast(user_unit, from_pos, direction, dmg_mul, shoot_player, spread_mul, autohit_mul, suppr_mul, target_unit)
		...
		for _, hit in ipairs(unique_hits) do
			if self._hit_player and not hit_player and shoot_player then
				...
				if player_hit then
					...
					local damaged_player = InstantBulletBase:on_hit_player(player_hit_ray, self._unit, user_unit, damage, self._fires_blanks)
	...
	if not shoot_player and self._suppression then
