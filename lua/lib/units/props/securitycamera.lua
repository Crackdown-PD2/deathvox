...
function SecurityCamera:set_detection_enabled(state, settings, mission_element)
	...
	if state then
		...
		if settings then
			...
			if settings.team_id then
				self._team = managers.groupai:state():team_data(settings.team_id)
			else
				self._team = managers.groupai:state():team_data(tweak_data.levels:get_default_team_ID("combatant"))
			end
	...
	if self._detection_interval < dt then
		...
		if self.update_position then
			...
			if self._look_fwd then
				...
				m_rot_y(tmp_rot1, self._look_fwd)
			end
		end
		...
		if managers.groupai:state()._draw_enabled then
			...
			mvec3_mul(cone_base, self._range)
			mvec3_add(cone_base, self._tmp_vec1)

			local cone_base_rad = math_tan(self._cone_angle * 0.5) * self._range
		...
		if self._is_server then
			...
			if not player then
				if #self._detected_attention_objects > 0 then
					self:_destroy_all_detected_attention_object_data()
				end
			...
			self:_upd_acquire_client_attention(t, player_key)
			self:_upd_detect_client_attention(t, player_key)
			self:_upd_local_client_suspicion(t, player_key)
		end
	end
end

function SecurityCamera:_upd_acquire_new_attention_objects(t)
	local all_attention_objects = managers.groupai:state():get_AI_attention_objects_by_filter(self._SO_access_str, self._team)
	...
	local min_reaction = AIAttentionObject.REACT_SUSPICIOUS
	...
	for u_key, attention_info in pairs(all_attention_objects) do
		if u_key ~= my_key and not detected_obj[u_key] then
			...
			if attention_info.unit:base() and attention_info.unit:base().is_husk_player then
				can_acquire = false
			end

			if can_acquire then
				local settings = attention_info.handler:get_attention(self._SO_access, min_reaction, nil, self._team)

				if settings then
					local attention_pos = attention_info.handler:get_detection_m_pos()
					local angle, dis_multiplier = self:_detection_angle_and_dis_chk(my_pos, my_fwd, attention_info.handler, settings, attention_pos)
					...
					if angle then
						...
						if not vis_ray or vis_ray.unit:key() == u_key then
							...
							if self._cone_angle ~= nil then
								local dir = attention_pos - my_pos
								dir = dir:normalized()
							...
							if in_cone then
								...
								detected_obj[u_key] = self:_create_detected_attention_object_data(t, u_key, attention_info, settings, visible_data)
							end
						end
					end
				end
...
function SecurityCamera:_upd_acquire_client_attention(t, player_key)
	...
	if not detected_obj[player_key] then
...
function SecurityCamera:_upd_detect_attention_objects(t)
	...
	for u_key, attention_info in pairs(detected_obj) do
		if attention_info.visible_in_this_instance or t >= attention_info.next_verify_t then
			local verify_interval = attention_info.settings.verification_interval

			if attention_info.identified and attention_info.verified then
				verify_interval = self._detection_interval
			else
				verify_interval = verify_interval * 0.3
			end
			...
			if not attention_info.identified then
				...
				if attention_info.visible_in_this_instance then
					...
					angle, dis_multiplier = self:_detection_angle_and_dis_chk(my_pos, my_fwd, attention_info.handler, attention_info.settings, attention_info.m_head_pos)

					if angle then
						local vis_ray = attention_info.visible_ray or self._unit:raycast("ray", my_pos, attention_info.m_head_pos, "slot_mask", self._visibility_slotmask, "ray_type", "ai_vision")
				...
				if noticable then
					if angle == -1 then
						...
						local angle_mul_mod = 0.15 * math_min(angle / self._cone_angle, 1)
					...
					if dis < max_dis * 1.2 then
						...
						if in_FOV then
							if attention_info.visible_in_this_instance then
								...
								local vis_ray = attention_info.visible_ray or self._unit:raycast("ray", my_pos, attention_pos, "slot_mask", self._visibility_slotmask, "ray_type", "ai_vision")
...
function SecurityCamera:_upd_detect_client_attention(t, player_key)
	local detected_obj = self._detected_attention_objects
...
function SecurityCamera:_detection_angle_and_dis_chk(my_pos, my_fwd, handler, settings, attention_pos)
	local dis = mvec3_dir(self._tmp_vec1, my_pos, attention_pos)
	...
	if dis_multiplier < 1 then
		if settings.notice_requires_FOV then
			local angle = mvec3_angle(my_fwd, self._tmp_vec1)
...
function SecurityCamera:_detection_angle_chk(my_pos, my_fwd, attention_pos, strictness)
	mvec3_dir(self._tmp_vec1, my_pos, attention_pos)
...
function SecurityCamera:get_mark_check_position()
	local obj = self._unit:get_object(Idstring("CameraLens")) or self._unit:get_object(Idstring("g_lamp")) or self._pos or self._unit:position()

	return obj:position()
end

function SecurityCamera:_create_detected_attention_object_data(t, u_key, attention_info, settings, visible_data)
	...
	local is_local_player, is_husk_player, is_deployable, is_person, char_tweak = nil
	...
	local dis = mvec3_dis(self._pos, m_pos)
	local verify_interval = settings.verification_interval * 0.3
	local new_entry = {
		verified = false,
		notice_progress = 0,
		settings = settings,
...
function SecurityCamera:_create_detected_client_data(t, u_key, attention_info, settings, visible_data)
	attention_info.handler:add_listener("sec_cam_" .. tostring(self._u_key), callback(self, self, "on_detected_attention_obj_modified"))

	local att_unit = attention_info.unit
	local m_head_pos = attention_info.handler:get_detection_m_pos()
	local dis = mvec3_dis(self._pos, m_head_pos)
	local new_entry = {
		notice_progress = 0,
		settings = settings,
		unit = attention_info.unit,
			...
			if not attention_data.verified then
				if attention_data.uncover_progress then
					...
					if attention_data.uncover_progress <= 0 then
						...
						max_suspicion = math_max(max_suspicion, attention_data.uncover_progress)
				...
				if attention_data.settings.uncover_range and dis < math_min(max_range, uncover_range) * susp_settings.range_mul then
				...
				elseif suspicion_range and dis < math_min(max_range, suspicion_range) * susp_settings.range_mul then
					if attention_data.last_suspicion_t then
						local dt = t - attention_data.last_suspicion_t
						local range_max = suspicion_range - uncover_range
						...
						if attention_data.uncover_progress then
							attention_data.uncover_progress = attention_data.uncover_progress + progress
						else
							attention_data.uncover_progress = progress
						end
...
function SecurityCamera:_upd_local_client_suspicion(t, player_key)
	local function _exit_func(attention_data)
		attention_data.unit:movement():on_uncovered(self._unit)
		self:_sound_the_alarm(attention_data.unit)
	end
...
function SecurityCamera:_set_suspicion_sound(suspicion_level)
	...
	if self._is_server then
		local suspicion_lvl_sync = math_clamp(math_ceil(suspicion_level * 6), 1, 6)
...
function SecurityCamera:_send_net_event(event_id, host_only)
	if host_only then
		managers.network:session():send_to_peer_synched(managers.network:session():peer(1), "sync_unit_event_id_16", self._unit, "base", event_id)
	else
...
function SecurityCamera:start_tape_loop(tape_loop_t)
	...
	elseif time_upgrade_level == 1 then
		self:_send_net_event(self._NET_EVENTS.request_start_tape_loop_1, true)
	elseif time_upgrade_level == 2 then
		self:_send_net_event(self._NET_EVENTS.request_start_tape_loop_2, true)
	end
end
...
function SecurityCamera:destroy(unit)
	table_delete(SecurityCamera.cameras, self._unit)
