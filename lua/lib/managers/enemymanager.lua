...
local task_throughput = 600 -- Default in FSS is 600. Going to roll with that for now.

function FullSpeedSwarmUpdateTaskThroughput(new_value)
	task_throughput = new_value
end

function EnemyManager:_update_queued_tasks(t, dt)
	local qt = self._queued_tasks
	local n = task_throughput == 0 and #qt or math_min(#qt, math_ceil(dt * task_throughput))
	local i_task = 1
	...
	function EnemyManager:_destroy_unit_gfx_lod_data(u_key)
		...
		for i = 1, nr_entries do
			...
			if u_key == unit:key() then
				local ub = unit:base()
				if ub._tweak_table == 'sniper' or ub._tweak_table == 'deathvox_sniper' then -- im pretty sure this enforces minimum LOD on snipers for optimization
...
function EnemyManager:on_enemy_unregistered(unit)
	...
	if u_mov._cur_seg and FullSpeedSwarm.units_per_navseg[u_mov._cur_seg] then
