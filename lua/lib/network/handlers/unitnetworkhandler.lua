...
if Network:is_server() then
	...
	function UnitNetworkHandler:damage_bullet(subject_unit, attacker_unit, ...)
		if alive(attacker_unit) and attacker_unit:in_slot(25) then -- turret
			if alive(subject_unit) and subject_unit:in_slot(16) then -- friendly AI
				local brain = subject_unit:brain()
				if brain and brain._logic_data and brain._logic_data.is_converted then
